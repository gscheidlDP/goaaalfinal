...
router.get('/:prodid', function(req, res, next) {
  let query = "SELECT product_name, product_desc, product_price, inv_count, prod_loc from product WHERE product_id = " + req.params.prodid ; 
...
let insertquery = "INSERT INTO product (product_name, product_desc, product_price, inv_count, prod_loc) VALUES (?, ?, ?, ?, ?, ?)"; 

	db.query(insertquery,[req.body.prod_id,req.body.product_name,req.body.product_desc,req.body.product_price,req.body.inv_count, req.body.prod_loc],(err, result) => {
	if (err) {
			console.log(err);
			res.render('error');
			} else {
...
router.post('/save', function(req, res, next) {
	...
	let updatequery = "UPDATE product set product_name = ?, product_desc = ?, product_price = ?, inv_count = ?, prod_loc = ? WHERE prod_id = " + req.body.product_id; 
...
router.get('/:prodid/delete', function(req, res, next) {
  let query = "DELETE from product WHERE prod_id = " + req.params.prodid ; 
